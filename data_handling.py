...
# Colours for the moment orders in a multi-k log-log scatter. Taken from a k's
MOMENT_COLOURS = ("#1f4e9c", "#2a9d8f", "#e07a1f", "#c0392b", "#7b52ab")
...
def confidence_interval(h_hat: float, n: int, lags: list[int],
                        level: float = CONFIDENCE_LEVEL) -> tuple[float, float, float]:
    """Plug-in asymptotic interval for H from the k = 2 CLT,

        sqrt(N) (H_hat_N - H) -> N(0, sigma^2_OLS(H)),

    i.e. H_hat +- z_{1-alpha/2} * sqrt(sigma^2_OLS(H_hat) / N). Returns
    (standard error, lower, upper). sigma^2_OLS only exists for 0 < H < 3/4, so
    an estimate outside that range gets NaNs rather than an interval."""
    if not 0 < h_hat < 0.75:
        return float("nan"), float("nan"), float("nan")
    se = float(np.sqrt(hpe.asymptotic_variance(lags, h_hat) / n))
    z = norm.ppf(0.5 + level / 2)
    return se, h_hat - z * se, h_hat + z * se


def hurst_table(series: dict[str, Series], lags: list[int], moments: list[float]) -> pd.DataFrame:
    """H_hat for every symbol at every moment order, plus the CLT-based
    confidence interval at k = CLT_MOMENT when that order is in `moments`."""
    k_grid = np.array(moments)
    ci_label = f"{CONFIDENCE_LEVEL:.0%}"
    rows = []
    for symbol, (asset_class, log_rv, dates) in series.items():
        ...
        if CLT_MOMENT in moments:
            h_clt = float(estimates[moments.index(CLT_MOMENT)])
            se, lower, upper = confidence_interval(h_clt, len(log_rv), lags)
            row.update({f"se(k={CLT_MOMENT})": se,
                        f"CI{ci_label}_low(k={CLT_MOMENT})": lower,
                        f"CI{ci_label}_high(k={CLT_MOMENT})": upper})
        row.update({"H_mono": h_mono,
                    "max_dev": float(np.abs(exponents - h_mono * k_grid).max())})
        rows.append(row)
    return pd.DataFrame(rows).set_index("symbol")


def moment_colour(k: float, moments: list[float]) -> str:
    """Colour for a moment order, by its position in `moments`."""
    idx = moments.index(k) if k in moments else 0
    return MOMENT_COLOURS[idx % len(MOMENT_COLOURS)]


def log_log_scatter(series: dict[str, Series], symbol: str, lags: list[int],
                    moments: list[float]):
    """log S_N(k, m) against log m for one symbol, one scatter and fitted line of
    slope k * H_hat per moment order. log RV is scaled here, which solely changes the intercepts in order to prevent 
    clutter."""
    _, log_rv, _ = series[symbol]
    log_lags = np.log(lags)

    incr = increments(LOG_VOL_FACTOR * log_rv, lags)
    ...
    for k in moments:
        ...
        colour = moment_colour(k, moments)
        ax.scatter(log_lags, log_moments, s=14, alpha=0.85, color=colour, zorder=3)
        ax.axline((0, intercept), slope=slope * k, color=colour, linewidth=1.2,
                  label=rf"$k={k:g}$, $\hat H={slope:.3f}$")

    ax.set_xlim(log_lags.min() - 0.15, log_lags.max() + 0.15)
    ax.set_xlabel(r"$\log(m)$")
    ax.set_ylabel(r"$\log(S_N(k,m))$")
    ax.set_title(f"{display_name(symbol)}: log-log scaling of absolute moments with OLS fits",
                 fontsize=11)
    ax.legend(loc="lower right", fontsize=8, title="moment order", title_fontsize=8,
              framealpha=0.9)
...
def main() -> None:
    ...
    for symbol in PLOT_SYMBOLS:
        figures = {
            "log_log_scatter": log_log_scatter(series, symbol, LAGS, MOMENTS),
